...
class TeamSelectionBloc
  ...
  List<Players> getTypeList(TabTextType tabTextType) {
    ...
    allPlayerList.forEach((player) {
      if (player.playingRole!.toLowerCase() ==
          getTypeText(tabTextType).toLowerCase()) {
  ...
  int getMax7PlayesCount(Players player) {
    ...
    allPlayerList.forEach((playerData) {
      if (player.teamName!.toLowerCase() ==
              playerData.teamName!.toLowerCase() &&
  ...
  int getSelectedPlayerCount({TabTextType? wk, TabTextType? tabTextType}) {
    ...
    allPlayerList.forEach((player) {
      if (wk == null) {
        ...
        if (player.isSelcted == true &&
            (player.playingRole!.toLowerCase() ==
                getTypeText(wk).toLowerCase())) {
          totalpoint += 1;
        }
      }
    });
  ...
  bool isDisabled(int pid) {
    ...
    allPlayerList.forEach((player) {
      if (playes.playingRole!.toLowerCase() ==
          player.playingRole!.toLowerCase()) {
    ...
    if (getMinMaxSelectionCount(getTypeTextEnum(playes.playingRole!)!)[1] <=
        selectedPlayes) {
      return true;
    } else {
      return false;
