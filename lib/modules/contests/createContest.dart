...
import 'package:tempalteflutter/utils/loadindDialogs.dart';
import 'package:tempalteflutter/validator/validator.dart';

import '../../api/create_context.dart';
...
class _CreateContestScreenState extends State<CreateContestScreen> {
                  ...
                  children: <Widget>[
                    ...
                    Container(
                      ...
                      child: Container(
                        ...
                        child: Opacity(
                          opacity:
                              finalEntryfee == 0.0 || isProsses ? 0.2 : 1.0,
                          child: Container(
                              ...
                              child: InkWell(
                                ...
                                onTap: () async {
                                  if (contestName != '') {
                                    ...
                                    showInSnackBar(
                                        'Please Enter Contest name.');
                                  }
                                  if (finalEntryfee != 0.0 &&
                                      isProsses == false) {
                                    List lst = [];
                                    lst.add(totalWinningamount / 5);
                                    lst.add(totalWinningamount / 6);
                                    lst.add(totalWinningamount / 7);
                                    lst.add(totalWinningamount / 8);
                                    lst.add(totalWinningamount / 9);

                                    for (int i = 0; i < lst.length; i++) {
                                      lst[i] = lst[i].toString().split('.')[0];
                                    }

                                    context_list c = context_list(
                                        entrycapacity: contestSize.toString(),
                                        name: contestName,
                                        id: widget.id,
                                        prizepool:
                                            totalWinningamount.toString(),
                                        entryfee: entryfee,
                                        entryleft: 0.toString(),
                                        entrycount: contestSize.toString(),
                                        winner: 0.toString(),
                                        pricelst: lst,
                                        firstprice: lst[0].toString());
                                    CreateContextAPI ccapi = CreateContextAPI();

                                    var data = await ccapi.post(c.tomap());
                                ...
                                child: Center(
                                  child: Text(
                                    finalEntryfee != 0.0
                                        ? 'Choose winning Breakup'.toUpperCase()
                                        : 'CREATE CONTEST',
                                    style: TextStyle(
                                      fontFamily: 'Poppins',
                                      fontWeight: FontWeight.bold,
  ...
  void setValidCalculation() {
    print(totalWinningamount);
    print(contestSize);
    if (totalWinningamount > 0 &&
        totalWinningamount <= 10000 &&
        contestSize > 1 &&
        contestSize <= 500) {
