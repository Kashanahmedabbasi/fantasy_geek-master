...
class LeagueTeamPreviewScreen extends StatefulWidget {
  ...
  @override
  _LeagueTeamPreviewScreenState createState() =>
      _LeagueTeamPreviewScreenState();
}
...
class _LeagueTeamPreviewScreenState extends State<LeagueTeamPreviewScreen> {
      ...
      if (data != null && data.success == 1 && data.playerList!.length > 0) {
        ...
        allPlayerList!.forEach((p) {
          if (p.title!.toLowerCase() ==
              leagueTeamData.leagueMember![0].captun!.toLowerCase()) {
            p.isSelcted = true;
            p.isC = true;
          }
          if (p.title!.toLowerCase() ==
              leagueTeamData.leagueMember![0].wiseCaptun!.toLowerCase()) {
            p.isVC = true;
            p.isSelcted = true;
          }
          if (isMach(p.title!)) {
        ...
        leagueTeamData.playerScore!.forEach((score) {
          allPlayerList.forEach((p) {
            if (p.shortName!.toLowerCase() == score.playerName!.toLowerCase() &&
                p.pid == int.parse(score.playerKey!)) {
              p.point = score.point;
            }
          });
        });
  ...
  bool isMach(String name) {
    ...
    allplayername
        .addAll(leagueTeamData.leagueMember![0].allRounder!.split(','));
  ...
  bool isMachStart() {
    var finaledate = DateFormat('yyyy-MM-dd HH:mm:ss').parse(
        widget.shedualData!.dateStart! + " " + widget.shedualData!.timeStart!);
                ...
                onTap: () {
                  if (!isMachStart()) {
                    ...
                    if (pData == null) {
                      ...
                      pData.points = 0.0;
                      pData.playerName =
                          player.lastName! + ' ' + player.middleName!;
