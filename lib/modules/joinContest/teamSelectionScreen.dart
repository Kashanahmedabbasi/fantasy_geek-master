...
class TeamSelectionScreen extends StatefulWidget {
  ...
  const TeamSelectionScreen(
      ...
      this.shedualData,
      this.leagueId,
      this.teamSelectionType = TeamSelectionType.regular,
      this.selectedTeamId = '',
      this.entryfee = '0',
...
class _TeamSelectionScreenState extends State<TeamSelectionScreen> {
                              ...
                              child: Container(
                                ...
                                child: Material(
                                  ...
                                  child: InkWell(
                                    ...
                                    onTap: () async {
                                      if (isTeamSelected()) {
                                        if (widget.teamSelectionType ==
                                            TeamSelectionType.regular) {
                                          setState(() {
                                            isProsses = true;
                                          });
                                          var userData = await ApiProvider()
                                              .drawerInfoList();
                                          setState(() {
                                            isProsses = false;
                                          });
                                          if (userData != null &&
                                              userData.data != null) {
                                            setshowDialog(widget.entryfee!,
                                                userData.data!);
                                          }
                                        } else if (widget.teamSelectionType ==
  ...
  void setshowDialog(String entryFee, UserData userData) {
    ...
    var unutilizedBalanceWinning =
        '${(double.tryParse(userData.winingAmount!)! + double.tryParse(userData.balance!)!).toStringAsFixed(2)}';
      ...
      children: <Widget>[
        Container(
          ...
          child: Text(
            ...
            textAlign: TextAlign.center,
            style: TextStyle(
                fontFamily: 'Poppins', fontSize: ConstanceData.SIZE_TITLE16),
          ),
        ),
        Divider(
          height: 1,
  ...
  Future getTeamDeatil() async {
    ...
    if (data.success == 1 && data.playerList!.length > 0) {
      ...
      players.forEach((p) {
        ...
        if (isMach(p.title!)) {
          if (widget.shedualData!.teamLogo!.a!.shortName!.toLowerCase() ==
              p.teamName!.toLowerCase()) {
            teamA += 1;
          }
          if (widget.shedualData!.teamLogo!.b!.shortName!.toLowerCase() ==
              p.teamName!.toLowerCase()) {
            teamB += 1;
          }
        }
